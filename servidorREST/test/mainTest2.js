...
describe( "Tarea 2: Funciones basicas de Admin", function() {
    ...
    it( "Busco el admin anteriormente introducido", function (hecho) {
        request.get(
            ...
            function (err, respuesta, carga) {
                ...
                assert.equal(solucion.dni_admin, "12345678A", "¿El dni del admin no es 12345678A?")
                hecho()
            } // callback
        ) //
    }) // it
    ...
    it( "Busco todos los admins", function (hecho) {
        request.get(
            ...
            function (err, respuesta, carga) {
                ...
                assert.equal(solucion[0].dni_admin, "12345678A", "¿El dni del admin no es 12345678A?")
                hecho()
            } // callback
        ) //
    }) // it
    ...
    it( "Borro admin por dni", function( hecho ) {
        var persona = { dni_admin: '12345678A' }
        request.post(
    ...
    it( "Compruebo que el admin se ha borrado", function (hecho) {
        request.get(
            ...
            function (err, respuesta, carga) {
                ...
                assert.equal( carga, "no encontré el admin con dni_admin: " + "12345678A", "¿No se ha borrado el admin?" )
                hecho()
            } // callback
        ) //
    }) // it
